...
class TokenPrompter(nn.Module):
    def __init__(self, prompt_len, latentdim=768) -> None:
        super(TokenPrompter, self).__init__()

        self.prompt = nn.Parameter(torch.randn([1, prompt_len, latentdim]))

    def forward(self):
        return self.prompt

class DeepTokenPrompter(nn.Module):
    def __init__(self, prompt_len) -> None:
        super(DeepTokenPrompter, self).__init__()

        self.prompt = [nn.Parameter(torch.randn([1, prompt_len, 768])) for _ in range(12)]
