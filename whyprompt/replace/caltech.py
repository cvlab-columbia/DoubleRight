...
class Caltech101(VisionDataset):
    ...
    def __init__(
        ...
        if split is not None:
            index_train, index_test, y_train, y_test = train_test_split(
                self.index, self.y, test_size=test_size, random_state=42, shuffle=True
            )
            self.y = y_train if split == 'train' else y_test
            self.index = index_train if split == 'train' else index_test

        clip_categories = self.categories.copy()
        clip_categories[1] = "person"
        self.prompt_template = prompt_template
        self.clip_prompts = [ 
            prompt_template.format(label.lower().replace('_', ' ').replace('-', ' ')) \
            for label in clip_categories[1:]
        ]
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        for t in self.target_type:
            if t == "category":
                temp_label = self.y[index]
                temp_label = temp_label-1 if temp_label >= 1 else temp_label
                target.append(temp_label)
